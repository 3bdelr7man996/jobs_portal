...
class IndustriesDropdown extends StatelessWidget {
  IndustriesDropdown({super.key});
    ...
    return Consumer<ProfileProvider>(builder: (context, profile, _) {
      return Container(
        ...
        child: DropdownButton<Configuration>(
          ...
          value: profile.userIndustryId != null
              ? homeProvider?.industries
                  ?.firstWhere((e) => e.id == profile.userIndustryId)
              : null,
          onChanged: (Configuration? v) {
            if (v != null) {
              context.read<ProfileProvider>().setIndustry(v.id!);
            }
          },
          items: homeProvider?.industries
