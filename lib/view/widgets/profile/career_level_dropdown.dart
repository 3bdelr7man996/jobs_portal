...
class CareerLevelDropdown extends StatelessWidget {
  CareerLevelDropdown({super.key});
    ...
    return Consumer<ProfileProvider>(builder: (context, profile, _) {
      return Container(
        ...
        child: DropdownButton<Configuration>(
          hint: Text("Select Career Level"),
          isExpanded: true,
          underline: Container(color: Colors.transparent),
          icon: Icon(Icons.arrow_drop_down),
          value: profile.userCarrerId != null
              ? homeProvider?.careerLevels
                  ?.firstWhere((e) => e.id == profile.userCarrerId)
              : null, //_selectedConfiguration,
          onChanged: (Configuration? v) {
            if (v != null) {
              context.read<ProfileProvider>().setCarrerId(v.id!);
            }
          },
          items: homeProvider?.careerLevels
